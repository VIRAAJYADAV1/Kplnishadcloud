...
# Owner: @kplboy
# Admin: VIRAAJ YADAV (@VIRAAJYADAV_247)
# ═══════════════════════════════════════════
...
ADMIN_USERNAME = "VIRAAJ YADAV (@VIRAAJYADAV_247)"
...
def main_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("👨‍💻 Admin", url="https://t.me/VIRAAJYADAV_247")],
...
def main():
    ...
    # Schedule promos every 3 hours (IST times)
    job_queue = app.job_queue
    if job_queue:
        job_queue.run_daily(send_promo_to_groups, time=time(hour=0, minute=30))   # 6:00 AM IST
        job_queue.run_daily(send_promo_to_groups, time=time(hour=3, minute=30))   # 9:00 AM IST
        job_queue.run_daily(send_promo_to_groups, time=time(hour=6, minute=30))   # 12:00 PM IST
        job_queue.run_daily(send_promo_to_groups, time=time(hour=9, minute=30))   # 3:00 PM IST
        job_queue.run_daily(send_promo_to_groups, time=time(hour=12, minute=30))  # 6:00 PM IST
        job_queue.run_daily(send_promo_to_groups, time=time(hour=15, minute=30))  # 9:00 PM IST
        job_queue.run_daily(send_promo_to_groups, time=time(hour=18, minute=30))  # 12:00 AM IST
        job_queue.run_daily(send_promo_to_groups, time=time(hour=21, minute=30))  # 3:00 AM IST
        logging.info("Promo schedule: Every 3 hours (6AM, 9AM, 12PM, 3PM, 6PM, 9PM, 12AM, 3AM IST)")
    ...
    print("═══════════════════════════════════════════")
    print("📢 Promo Schedule: Every 3 hours")
    print("═══════════════════════════════════════════")
